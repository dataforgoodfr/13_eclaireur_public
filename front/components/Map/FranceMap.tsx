...
export default function FranceMap({
	...
	const departementsMaxZoomThreshold =
		selectedTerritoryData?.departementsMaxZoom || 8;
	const communesMaxZoomThreshold =
		(selectedTerritoryData?.communesMaxZoom || 14) + 1; // +1 because maxzoom is exclusive
	...
	const choroplethParameter =
		selectedChoroplethData.dataName || "subventions_score";
	const { data: communes, isLoading: communesLoading } = useCommunes(
		visibleCommuneCodes,
		selectedYear,
	);
	const { data: departements, isLoading: departementsLoading } =
		useDepartements(visibleDepartementCodes, selectedYear);
	...
	return (
		<div className="relative h-full w-full cursor-grab bg-white">
			...
			<Map
				...
				touchPitch={false}
				maxBounds={
					territoryFilterCode === "FR" ? franceMetropoleBounds : undefined
				}
				onLoad={() => {
					const mapInstance = mapRef.current?.getMap();
					if (mapInstance) {
