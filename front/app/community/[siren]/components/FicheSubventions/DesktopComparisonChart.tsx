...
const CommunityBar = (props: unknown) => {
  ...
  const strokeColor = payload?.communityMissing ? '#E5C72E' : '#E8F787';
  const radius = 12;
            ...
            return (
              <div key={index} className='flex items-center gap-3'>
                {isStriped ? (
                  ...
                  <div className='h-4 w-4 flex-shrink-0 rounded bg-brand-2' />
                )}
                <div className='flex-1'>
                  <p className='text-sm text-gray-700'>{entry.name}</p>
                  <p className='text-base font-bold text-brand-2'>
