...
export function CustomDataTableToolbar<TData>({
  ...
  className,
  ...props
}: CustomDataTableToolbarProps<TData>) {
  ...
  return (
    ...
    >
      <div className='flex flex-1 flex-wrap items-center gap-2'>
        {columns.map((column) => (
          <CustomDataTableToolbarFilter key={column.id} column={column} />
        ))}
        {isFiltered && (
          <Button
...
interface CustomDataTableToolbarFilterProps<TData> {
  column: Column<TData>;
}

function CustomDataTableToolbarFilter<TData>({ column }: CustomDataTableToolbarFilterProps<TData>) {
