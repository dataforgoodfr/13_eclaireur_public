...
# Appliquer le calcul
df["aggregated_score"] = df.apply(
    lambda row: calculate_aggregated_score(row["score_sub"], row["score_marches_pub"]), axis=1
)
...
for score in ["A", "B", "C", "D", "E", "UNKNOWN"]:
    ...
    if not examples.empty:
        ...
        for _, row in examples.iterrows():
            print(
                f"{row['Collectivite']} (sub: {row['score_sub']}, mp: {row['score_marches_pub']})"
            )
