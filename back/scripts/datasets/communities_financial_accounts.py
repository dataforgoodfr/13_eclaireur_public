...
class FinancialAccounts(DatasetAggregator):
    ...
    def __init__(self, files: pd.DataFrame, main_config: dict):
        ...
        self.columns_mapping = pd.read_csv(
            main_config[self.get_config_key()]["columns_mapping"], sep=";"
        ).set_index("name")

        last_updates = self.files_in_scope["datasetid"].apply(
            self._retrieve_last_modification_date
        )
        ...
        if resp.status_code == 200:
            ...
            if (
                ...
                return datetime.strptime(
                    resp_j["metas"]["default"]["modified"], "%Y-%m-%dT%H:%M:%S.%f%z"
                ).replace(tzinfo=None)
        return last_updated
