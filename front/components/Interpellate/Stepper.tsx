...
export default function Stepper({ currentStep, numberOfSteps = 4 }: StepperProps) {
  ...
  return (
    <div className='global-margin md:pl-ms-0 flex flex-nowrap justify-stretch gap-0 pl-8 md:gap-2'>
      {Array.from({ length: numberOfSteps }).map((_, index) => (
        <div key={index} className='flex basis-[15%] items-baseline font-kanit-bold'>
